...
const TermsAndConditionsScreen = ({ navigation }: StackPropsType<"TermsAndConditions">) => {
    ...
    useEffect(() => {
        dispatch(getCmsData("terms-conditions"))
        .unwrap()
        .then((res: any) => {
          let resp = res?.data ? res.data : res;
          setTermsData(resp?.content)
          console.log('terms & condition data', resp?.content);
        });
    },[]);
    ...
    return (
      <CustomScreen isSafe customStyle={{alignItems: 'center'}}>
        ...
        title={LABELS.TermsAndConditionsScreen.header}
        showBack
      />
