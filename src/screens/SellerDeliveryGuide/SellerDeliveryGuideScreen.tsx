...
const SellerDeliveryGuideScreen = ({ navigation }: StackPropsType<"SellerDeliveryGuide">) => {
    ...
    useEffect(() => {
        dispatch(getCmsData("seller-s-guidelines"))
        .unwrap()
        .then((res: any) => {
          let resp = res?.data ? res.data : res;
          setTermsData(resp?.content)
          console.log('seller guide data', resp?.content);
        });
    },[]);
    ...
    return (
      <CustomScreen isSafe customStyle={{alignItems: 'center'}}>
       <CustomHeader
        ...
        title={LABELS.SellerDeliveryGuideScreen.header}
        showBack
      />
  ...
  export default SellerDeliveryGuideScreen;